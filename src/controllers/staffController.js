...
exports.createStaff = async (req, res, next) => {
  ...
  try {
    ...
    if (staffExists) {
      return res
        .status(400)
        .json({ message: "This user is already registered as Staff" });
    }
    ...
    if (memberExists) {
      return res
        .status(400)
        .json({ message: "This user is already registered as Member" });
    }
    ...
    if (telpExistsInStaff) {
      return res
        .status(400)
        .json({ message: "Phone number is already registered to a Staff" });
    }
    ...
    if (telpExistsInMember) {
      return res
        .status(400)
        .json({ message: "Phone number is already registered to a Member" });
    }
  ...
  try {
    ...
    if (housecode || city || province || posCode || rtRw) {
      ...
      if (!address) {
        return res
          .status(404)
          .json({ message: "Address not found for this Staff" });
      }
